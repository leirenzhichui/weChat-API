...
/*
 * 微信官方API操作
 * 参考文档：http://mp.weixin.qq.com/wiki/index.php
 * 功能 : 微信公众平台API操作
 * 被动回复消息;设置菜单;获取用户列表;获取用户分组等
 * $this->log()函数可以自己写一个调试函数
 * @author zhuwei<zhuwei313@hotmail.com>
 * @version 2.0
 * $Id: Wechat.php 2915 2014-11-21 16:37:11Z A1256 $
 */
class Wechat
{
    ...
    const MSGTYPE_EVENT = 'event';
	const MSGTYPE_TRANSFER_CUSTOMER_SERVICE = 'transfer_customer_service';  //新增多客服服务；
    ...
    const EVENTTYPE_VIEW = 'VIEW'; //新增链接点击事件 140317
    const EVENTTYPE_MASSSENDJOBFINISH = 'MASSSENDJOBFINISH'; //新增群发消息完成事件 140317
    ...
    public function Message($msg = '',$append = false){
            if (is_null($msg)) {
                ...
                if ($append)
                    $this->_msg = array_merge($this->_msg,$msg);//合并消息
                else
                    $this->_msg = $msg;
                return $this->_msg;
            } else {
     ...
     * @该函数是接收微信消息体的首要处理函数，可以指定xmldata进行调试
     * @调试的xml消息可以是明文，密文，兼容模式
        * 调试的xml:<xml><ToUserName><![CDATA[gh_38aaebf8891f]]></ToUserName>
        <FromUserName><![CDATA[oFDart7kN7X-omd64dfOehWfo0f0]]></FromUserName>
        <CreateTime>1416310976</CreateTime>
        <MsgType><![CDATA[event]]></MsgType>
        <Event><![CDATA[CLICK]]></Event>
        <EventKey><![CDATA[qq123]]></EventKey>
        </xml>'
     */
    ...
    */
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        if ($result)
        {
            ...
            //和上面的一样主要是设置appid,access_token时间，防止再次查询数据
    ...
    public function resetAuth($appid=''){
        ...
        $this->access_token = '';
        //Cache::wechatToken($appid,null);//清理缓存中的appid信息
        return true;
    }
    ...
    public function uploadMedia($address, $type){
        ...
        // 新的http_post
         $result = $this->http_post(self::UPLOAD_MEDIA_URL.self::MEDIA_UPLOAD.'access_token='.$this->access_token.'&type='.$type,$param,true);//第三个参数是媒体上传
        if ($result) {
            $json = json_decode($result, true);
    ...
    public function getQRUrl($ticket) {
        return self::QRCODE_IMG_URL.urlencode($ticket);//建议编码
    }
     ...
     * @return boolean|array
     */
    public function getCustomServiceMessage($starttime,$endtime,$openid='',$pagesize=10,$pageindex=1){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
                'starttime' => $starttime,
                'endtime' => $endtime,
